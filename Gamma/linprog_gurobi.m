function [x, fval, exitflag, result] = linprog_gurobi(f, A, b, Aeq, beq, lb, ub, start_val, cutoff_val)
...
params.outputflag = 0;
params.TimeLimit = 20;% = 0.5 min limit for training


if ~isempty(start_val)
    model.start = start_val;
    model.cutoff = cutoff_val;
    %params.SOLUTION_LIMIT  = 1;
end
result = gurobi(model, params);

if ~(strcmp(result.status, 'OPTIMAL'))
    display(['gamma optim: flag ', result.status]);
end
%display(['gamma optim: obj value: ', num2str(result.objval), ', time = ', num2str(result.runtime)]);
